...
################################################################################
    ...
    def fenetre_infos_resa (self,resa) :
        ...
        est_reglee.set(1 if (resa._facture and resa._facture._date_de_reglement) else 0)
        liste_num_ch = []
        liste_nb_occupants = []
        ...
        Label(master_infos, 
                text=str(resa._facture._date_de_reglement.strftime("%d-%m-%Y")) if (resa._facture and resa._facture._date_de_reglement) else "" ,fg = COUL_POLICE_CHAMPS, font = font.Font(family = POLICE_CHAMPS, size =POLICE_CHAMPS_TAILLE)).grid(row= 4,column = 1, sticky=E)
        ...
        def sauvegarder_infos ():
            ...
            if est_reglee.get() == 1 and (resa._facture and resa._facture._date_de_reglement==None):
